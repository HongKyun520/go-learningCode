...
// SetupSuite 在整个测试套件开始前进行设置
// 主要完成以下工作:
// 1. 初始化MongoDB连接并获取数据库实例
// 2. 获取articles和published_articles集合的引用
// 3. 初始化雪花算法节点用于生成ID
// 4. 创建文章处理器实例
// 5. 设置gin服务器和中间件
// 6. 注册路由
func (s *ArticleMongoDBHandlerSuite) SetupSuite() {
	...
	// 创建文章处理器实例
	mongodbArticleDAO := dao.NewMongoDBArticleDAO(s.mdb, node)

	// todo
	hdl := startup.InitArticleHandler(mongodbArticleDAO)
...
// TearDownTest 在每个测试用例执行完毕后清理测试数据
// 主要完成以下工作:
// 1. 创建一个带超时的context,超时时间为1秒
// 2. 删除articles集合中的所有文档
// 3. 删除published_articles集合中的所有文档
// 4. 确保删除操作没有发生错误
